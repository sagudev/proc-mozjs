...
impl TryToTokens for ast::Export {
    fn try_to_tokens(self: &ast::Export, into: &mut TokenStream) -> Result<(), Diagnostic> {
        ...
        let max_nargs = self.function.arguments.len() as u32;
        //let o_nargs = self.function.arguments.iter().filter(|&t| *t.ty == 91).count();
        let attrs = &self.function.rust_attrs;

        (quote! {
            #[allow(non_snake_case)]
            #[allow(clippy::all)]
            unsafe extern "C" fn #generated_name(context: *mut ::mozjs::jsapi::JSContext, argc: u32, vp: *mut ::mozjs::jsapi::Value) -> bool { //(#(#args),*)
                let args = ::mozjs::jsapi::CallArgs::from_vp(vp, argc);
                args.rval().set(::mozjs::jsval::UndefinedValue());
                
                if args.argc_ == #max_nargs {
                    let #ret = {
                        #(#arg_conversions)*
