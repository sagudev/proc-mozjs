...
/// Parsed attributes from a `#[mozjs(..)]`.
    ...
    ($(($name:ident, $variant:ident($($contents:tt)*)),)*) => {
        ...
        fn check_used(self) -> Result<(), Diagnostic> {
            ...
            for (used, attr) in self.attrs.iter() {
                ...
                errors.push(Diagnostic::span_error(*span, "unused #[mozjs] attribute"));
            }
...
impl BindgenAttrs {
    /// Find and parse the mozjs attributes.
    fn find(attrs: &mut Vec<syn::Attribute>) -> Result<BindgenAttrs, Diagnostic> {
        ...
        loop {
            let pos = attrs
                .iter()
                .enumerate()
                .find(|&(_, ref m)| m.path.segments[0].ident == "mozjs")
                .map(|a| a.0);
            let pos = match pos {
                Some(i) => i,
                None => return Ok(ret),
            ...
            let group = match tts.next() {
                ...
                Some(_) => bail_span!(attr, "malformed #[mozjs] attribute"),
                None => continue,
            };
            if tts.next().is_some() {
                bail_span!(attr, "malformed #[mozjs] attribute");
            }
...
macro_rules! gen_bindgen_attr {
    ($(($method:ident, $($variants:tt)*),)*) => {
        /// The possible attributes in the `#[mozjs]`.
        #[cfg_attr(feature = "extra-traits", derive(Debug, PartialEq, Eq))]
...
/// (`Ctx`) is used to pass in the attributes from the `#[mozjs]`, if needed.
...
impl<'a> ConvertToAst<BindgenAttrs> for &'a mut syn::ItemStruct {
    ...
    fn convert(self, attrs: BindgenAttrs) -> Result<Self::Target, Diagnostic> {
        if self.generics.params.len() > 0 {
            bail_span!(
                self.generics,
                "structs with #[mozjs] cannot have lifetime or \
...
impl ConvertToAst<BindgenAttrs> for syn::ItemFn {
    ...
    fn convert(self, attrs: BindgenAttrs) -> Result<Self::Target, Diagnostic> {
        match self.vis {
            ...
            _ => bail_span!(self, "can only #[mozjs] public functions"),
        }
        if self.sig.constness.is_some() {
            bail_span!(
                self.sig.constness,
                "can only #[mozjs] non-const functions"
            );
        }
        /*
        if self.sig.unsafety.is_some() {
            bail_span!(self.sig.unsafety, "can only #[mozjs] safe functions");
        }
        */
        assert_not_variadic(&attrs)?;
...
fn function_from_decl(
    ...
    if sig.variadic.is_some() {
        bail_span!(sig.variadic, "can't #[mozjs] variadic functions");
    }
    if sig.generics.params.len() > 0 {
        bail_span!(
            sig.generics,
            "can't #[mozjs] functions with lifetime or type parameters",
        );
    }
...
pub(crate) trait MacroParse<Ctx> {
    ...
    ///
    /// The context is used to have access to the attributes on `#[mozjs]`, and to allow
    ...
    fn macro_parse(
        ...
        match self {
            ...
            _ => {
                bail_span!(
                    self,
                    "#[mozjs] can only be applied to a function, \
...
impl<'a> MacroParse<BindgenAttrs> for &'a mut syn::ItemImpl {
    fn macro_parse(
        ...
        if self.defaultness.is_some() {
            bail_span!(
                self.defaultness,
                "#[mozjs] default impls are not supported"
            );
        }
        if self.unsafety.is_some() {
            bail_span!(
                self.unsafety,
                "#[mozjs] unsafe impls are not supported"
            );
        }
        if let Some((_, path, _)) = &self.trait_ {
            bail_span!(path, "#[mozjs] trait impls are not supported");
        }
        if self.generics.params.len() > 0 {
            bail_span!(
                self.generics,
                "#[mozjs] generic impls aren't supported"
            );
        }
        let name = match get_ty(&self.self_ty) {
            ...
            _ => bail_span!(
                self.self_ty,
                "unsupported self type in #[mozjs] impl"
            ),
        };
        let mut errors = Vec::new();
...
// internal attribute, `__mozjs_class_marker`, with any metadata we need
// to pass from the impl to the impl item. Recursive macro expansion will then
// expand the `__mozjs_class_marker` attribute.
//
...
fn prepare_for_impl_recursion(
    ...
    let method = match item {
        ...
        syn::ImplItem::Const(_) => {
            bail_span!(
                &*item,
                "const definitions aren't supported with #[mozjs]"
            );
        }
        syn::ImplItem::Type(_) => bail_span!(
            &*item,
            "type definitions in impls aren't supported with #[mozjs]"
        ),
    ...
    method.attrs.insert(
        ...
        syn::Attribute {
            ...
            path: syn::parse_quote! { mozjs::prelude::__mozjs_class_marker },
...
fn import_enum(enum_: syn::ItemEnum, program: &mut ast::Program) -> Result<(), Diagnostic> {
    ...
    for v in enum_.variants.iter() {
        match v.fields {
            syn::Fields::Unit => (),
            _ => bail_span!(v.fields, "only C-Style enums allowed with #[mozjs]"),
        }
        ...
        match get_expr(expr) {
            ...
            expr => bail_span!(
                expr,
                "enums with #[mozjs] cannot mix string and non-string values",
            ),
        }
    }
...
impl<'a> MacroParse<(&'a mut TokenStream, BindgenAttrs)> for syn::ItemEnum {
    fn macro_parse(
        ...
        match self.vis {
            syn::Visibility::Public(_) => {}
            _ => bail_span!(self, "only public enums are allowed with #[mozjs]"),
        }
                    ...
                    Some((_, expr)) => match get_expr(expr) {
                        syn::Expr::Lit(syn::ExprLit {
                            ...
                            Err(_) => {
                                bail_span!(
                                    int_lit,
                                    "enums with #[mozjs] can only support \
                        ...
                        expr => bail_span!(
                            expr,
                            "enums with #[mozjs] may only have \
...
impl MacroParse<BindgenAttrs> for syn::ItemConst {
    fn macro_parse(self, program: &mut ast::Program, opts: BindgenAttrs) -> Result<(), Diagnostic> {
        ...
        if opts.typescript_custom_section().is_none() {
            bail_span!(self, "#[mozjs] will not work on constants unless you are defining a #[mozjs(typescript_custom_section)].");
        }
        ...
        match get_expr(&self.expr) {
            ...
            expr => {
                bail_span!(expr, "Expected a string literal to be used with #[mozjs(typescript_custom_section)].");
            }
        }
